...
const HomePage = () => {
  ...
  const handleSearchSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`http://100.25.222.45/api/v1/service_providers?category=${category}&location=${location}`);
  ...
  const handleLogout = async () => {
    try {
      await axios.post('http://100.25.222.45/api/v1/logout');
